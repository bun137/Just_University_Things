...
void infix_to_postfix(char *inexp, char *outexp){
    ...
    while(token != '\0'){
        if(isalnum(token)){ // to check if token is an operand -> 'a,b,c,...., z , 1,2,3,...., inf' , is it? alphabet or num
            outexp[k++] = token;
        }else{ // it is an operator
            if (token == ')'){ // right bracket -> go to the stack and remove all the operands till you reach the left bracket
                while(peek(&stack) != '('){
                    outexp[k++] = pop(&stack);
                }
                pop(&stack);
        ...
        token = inexp[++i];
    } // comes out of the while loop bc token = '\0', there might be stuff left in the stack
    while(peek(&stack)!='#'){
        outexp[k++] = pop(&stack);
    }
    outexp[k]='\0';
