...
void insertion_pos(int data, NODEE **head)
{
    ...
    else
    {
        ...
        for (i; dhead != NULL; i++)
        {
            ...
            {
                printf("\n you have reached the end of the sll, doesnt matter if i == pos + 1, I'll still insert you at the end of the list, also i = %d \n", i);
                dhead->ptr_to_next = newnode; // one sec I could have just used insertback function bruhh
                newnode->ptr_to_next = NULL;
                return;
            }
            dhead = dhead->ptr_to_next;
